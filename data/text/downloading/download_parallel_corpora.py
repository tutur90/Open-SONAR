...
def download_TICO(directory, verbose=False):
    ...
    target_langs = {
        ...
        "sw": "swh",
        "ta": "tam",
        "ti_ET": "tir_Ethi",  # we combine both Tigrinya varieties
...
def download_from_opus(directory, opus_name, download_url):
    dataset_directory = os.path.join(directory, opus_name)
    os.makedirs(dataset_directory, exist_ok=True)
    print(f"Saving {opus_name} data to:", dataset_directory)

    response = requests.get(download_url)
    if not response.ok:
        print(f"Could not download from {download_url} ... aborting for {opus_name}!")
        return
    download_path = os.path.join(dataset_directory, download_url.split("/")[-1])
    open(download_path, "wb").write(response.content)
    print(f"Wrote: {download_path}")
    ...
    eng_file = os.path.join(dataset_directory, f"{opus_name}.")
    os.rename(
...
def download_XhosaNavy(directory):
    ...
    download_url = (
        "https://object.pouta.csc.fi/OPUS-XhosaNavy/v1/moses/en-xh.txt.zip"
    )
    response = requests.get(download_url)
    if not response.ok:
...
def download_Bianet(directory):
    ...
    """
    dataset_directory = os.path.join(directory, "bianet", "eng-kur")
    os.makedirs(dataset_directory, exist_ok=True)
    print("Saving Bianet data:", dataset_directory)

    # eng-kur
    download_url = "https://object.pouta.csc.fi/OPUS-Bianet/v1/moses/en-ku.txt.zip"
    opus_name = "bianet"

    extract_directory = download_from_opus(dataset_directory, opus_name, download_url)

    eng_file = os.path.join(dataset_directory, f"{opus_name}.eng")
    os.rename(
        os.path.join(extract_directory, "Bianet.en-ku.en"),
        eng_file,
    )
    print(f"Wrote: {eng_file}")

    kur_file = os.path.join(dataset_directory, f"{opus_name}.kur")
    os.rename(
        os.path.join(extract_directory, "Bianet.en-ku.ku"),
        kur_file,
    )
    print(f"Wrote: {kur_file}")
    ...
    dataset_directory = os.path.join(directory, "bianet", "kur-tur")
    os.makedirs(dataset_directory, exist_ok=True)

    # kur-tur
    download_url = "https://object.pouta.csc.fi/OPUS-Bianet/v1/moses/ku-tr.txt.zip"
    
    extract_directory = download_from_opus(dataset_directory, opus_name, download_url)

    eng_file = os.path.join(dataset_directory, f"{opus_name}.tur")
    os.rename(
        os.path.join(extract_directory, "Bianet.ku-tr.tr"),
        eng_file,
    )
    print(f"Wrote: {eng_file}")

    xho_file = os.path.join(dataset_directory, f"{opus_name}.kur")
    os.rename(
        os.path.join(extract_directory, "Bianet.ku-tr.ku"),
        xho_file,
    )
    print(f"Wrote: {xho_file}")
